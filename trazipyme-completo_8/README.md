...
### Novedades de esta vuelta (v2.3)

- **Historial de pagos real**: nueva tabla `pagos`, alimentada automáticamente
  por el webhook de MercadoPago cada vez que procesa un cobro. El dashboard
  del cliente y la ficha de cada cliente en `/admin` muestran ese historial
  (fecha, monto, estado, método) sin tener que consultar la API de
  MercadoPago en vivo.
- **El cliente puede cancelar su propia suscripción**: nueva función
  `cancelar-suscripcion-mercadopago`, con botón correspondiente en el
  dashboard (solo visible si la suscripción está activa o con pago
  pendiente).
- **Monitoreo real en `/admin`**: barra de indicadores arriba de la tabla de
  clientes (clientes totales, suscripciones activas, en prueba, y el MRR —
  ingreso mensual recurrente). La tabla de clientes ahora tiene buscador por
  nombre, y las pruebas que vencen en 3 días o menos se resaltan en rojo.

### Novedades de esta vuelta (v2.4)

- **Se quitó "Deshacer último escaneo"** de la app del operario (botón y
  función completa, no quedó código muerto).
- **Eliminar un QR**: nueva función `eliminar_producto()` — cualquier
  miembro del tenant (operario incluido) puede eliminar un producto desde
  su ficha, tanto en `/app` como en `/dashboard`. Es un *soft-delete*
  (`activo=false`): el QR desaparece de las listas activas pero su
  historial se conserva para siempre.
- **Auditoría completa del ciclo de vida del producto**: además de los
  movimientos de stock (entrada/salida/muestra), el historial ahora también
  registra automáticamente cuándo se **creó**, **editó** o **eliminó** un
  QR, con el detalle exacto de qué cambió y quién lo hizo — visible tanto
  en el dashboard del dueño como en la ficha de cliente de `/admin`.
- **Suspender/reactivar una cuenta de cliente**: nuevo botón en `/admin`
  (usa la columna `tenants.activo`, que ya existía pero no se exponía). A
  diferencia de una suscripción vencida (que solo bloquea nuevos
  movimientos), una cuenta suspendida bloquea el acceso completo — el
  dueño/gerente ve una pantalla de "cuenta suspendida" al iniciar sesión.
- **Edición de datos del cliente** (nombre/RUT) directamente desde su ficha
  en `/admin`.
- **Landing pública separada del panel interno**: se quitó el link directo
  a `/admin` de la página de inicio (`/index.html`) — antes cualquier
  visitante veía el botón "Panel Odyssai" junto a los de cliente. Ahora la
  landing solo muestra las 2 puertas de entrada de cliente; `/admin` sigue
  funcionando normal para quien ya conoce la URL directa.
- **Limpieza de vestigios**: se eliminaron `deshacer_movimiento()` (ya no la
  llama nadie) y `touch_tenant_period()` (una función muerta de la época de
  Stripe que ni siquiera compilaría hoy, porque referenciaba columnas ya
  eliminadas).

### Novedades de esta vuelta (v2.5)

- **Bug real corregido: `admin@odyssai.cl` podía entrar a `/dashboard`**.
  Como la cuenta super_admin no pertenece a ninguna empresa, RLS le mostraba
  una mezcla de productos de **todos** los clientes a la vez (confuso, y
  potencialmente delicado), y cualquier acción (editar, eliminar, registrar
  movimiento) fallaba con "No autenticado o sin tenant asignado" porque las
  funciones RPC correctamente no saben a qué tenant aplicar la acción. Ahora
  `/dashboard` rechaza a las cuentas `super_admin` directamente con un
  mensaje que las manda a `/admin`, que es donde deben trabajar.

### Novedades de esta vuelta (v2.6)

- **Eliminar clientes (archivar)**: Odyssai puede "eliminar" un cliente desde
  su ficha. No borra nada: lo archiva — sale de la lista principal y de los
  KPIs/MRR, se le cancela la suscripción y su equipo pierde acceso, pero toda
  su información (equipo, bodegas, actividad, pagos) sigue consultable
  marcando **"Ver clientes archivados"**. Se puede restaurar en cualquier
  momento.
- **Filtro y orden de QR** en las dos superficies (app de operario y
  dashboard del dueño), con la misma semántica en ambas: filtrar por
  con alertas / stock bajo / por vencer / sin alertas, y ordenar por nombre,
  stock, vencimiento más próximo, mayor valor o más recientes. Las tarjetas
  ahora muestran stock y sus alertas directamente.
- **Alertas clickeables en la app del operario**: el resumen del inicio ahora
  muestra chips separados ("📉 3 con stock bajo", "⏰ 2 por vencer") que
  llevan a la grilla de QR ya filtrada por esa condición. En el dashboard del
  dueño ya lo eran.
- **Bodegas administrables por el cliente**: el selector de bodegas incluye
  **"➕ Añadir bodega..."**, que lleva a *Suscripción y pagos* — ahí el
  cliente contrata o quita bodegas adicionales (el monto se recalcula solo:
  +$20.000/mes cada una) y ve su método de pago. Se agregó el cupo
  `bodegas_contratadas` (lo que paga) separado de las bodegas que alcanzó a
  crear; crear una bodega por sobre el cupo se bloquea **en la base de
  datos**, con un mensaje que explica cómo ampliarlo.
- **Permisos de bodega por usuario**: nueva sección "Bodegas y accesos" en el
  dashboard. Dueños y gerentes ven todas las bodegas siempre; a cada operario
  se le da acceso bodega por bodega con una casilla. El filtrado es real, a
  nivel de RLS: un operario sin acceso no puede ver esa bodega, ni sus
  productos, ni sus movimientos — no es solo la UI escondiéndolos.
...
- **Lógica de negocio crítica en RPC (Postgres functions)**, no en el cliente:
  `registrar_movimiento`, `crear_producto`, `actualizar_producto`,
  `eliminar_producto`, `crear_invitacion`, `calcular_monto_mensual`. Esto
...
- **operator**: usa `/app`. Escanea, registra entradas/salidas/muestras, crea
  y **elimina** productos (soft-delete, conserva el historial), ve historial
  y la grilla de QRs. No puede editar campos de un producto existente (eso
  es solo de dueño/gerente), y no entra al dashboard.
- **manager** / **owner**: todo lo anterior + `/dashboard` completo (reportes,
  alertas, invitar usuarios, editar productos, gestionar suscripción y pagos).
  `owner` es el dueño de la cuenta; `manager` es un gerente con los mismos
  permisos operativos.
- **super_admin**: staff de Odyssai. Entra a `/admin` (acceso completamente
  separado, con su propia sesión de navegador — nunca se mezcla con la de
  clientes). Crea y administra todos los tenants: edita sus datos, gestiona
  su suscripción, y puede suspender/reactivar una cuenta completa.
...
9. **(v2.3) Cancelación de suscripción**: un dueño de prueba con
   suscripción `active` llama `cancelar-suscripcion-mercadopago` → queda en
   `canceled`, verificado directamente en la base.
10. **(v2.3) RLS de la tabla `pagos`**: ese mismo dueño de prueba, con 2
    pagos cargados, consulta `/rest/v1/pagos` con su propio token → ve
    exactamente sus 2 pagos (ni más, ni de otro tenant).
11. **(v2.4) Auditoría de creación/edición/eliminación**: un operario crea
    un producto → queda `CREACION`. Ese mismo operario intenta editarlo →
    sigue bloqueado (correcto, solo dueño/gerente edita). La dueña lo edita
    → queda `EDICION` con el detalle exacto del cambio. El operario lo
    elimina → funciona (a diferencia de editar, cualquiera puede eliminar),
    el producto queda `activo=false` y se registra `ELIMINACION`.
12. **(v2.4) Suspensión de cuenta**: marqué un tenant de prueba como
    `activo=false` y verifiqué el cambio directo en la base.
...
de filas de las tablas están intactos (verifiqué también que un tenant real
llamado "Juan" que ya habías creado tú mismo mientras yo trabajaba siguiera
intacto, sin tocarlo).

⚠️ **Nota aparte, no relacionada con este código**: mientras probaba,
`trazpyme.vercel.app` empezó a devolver `DEPLOYMENT_NOT_FOUND` (un error de
Vercel, no de la app) — el deployment activo para ese dominio desapareció.
Vas a necesitar redesplegar de todas formas para que te lleguen los cambios
de esta vuelta, así que revisa el estado del proyecto en tu dashboard de
Vercel al hacerlo.
