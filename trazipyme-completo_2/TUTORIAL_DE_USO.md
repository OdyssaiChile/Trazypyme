...
### Deshacer tu último escaneo
- Menú principal → **Deshacer último escaneo**.
- Esto **solo revierte tu propio último movimiento** — si otro operario registró algo después que tú, no lo toca. Cada operario solo puede deshacer lo suyo (tu gerente sí puede corregir movimientos de cualquiera desde el dashboard, por si se necesita).

### Ver historial
- Menú inferior → **Historial**: los últimos 30 movimientos de tu bodega activa.
...
- **Historial completo** de ese producto (no solo los últimos 15).
...
*(Nota: esta edición directa está disponible solo para dueños/gerentes — los operarios ven la misma ficha pero sin poder editarla, solo consultarla.)*
...
Sección "Suscripción": ves tu plan (Prueba gratuita o Pro), estado, el **monto mensual calculado automáticamente** (base $50.000 + $20.000 por cada bodega adicional a las 2 incluidas), y fecha de próximo cobro o fin de prueba. El botón **Activar / renovar plan** genera un link de pago de MercadoPago — ingresas tu tarjeta una vez y queda con cobro automático cada mes, sin volver a pagar manualmente.
...
La tabla "Clientes existentes" muestra empresa, estado, **monto mensual calculado** y fecha de próximo cobro/fin de prueba. **Haz clic en cualquier fila** para abrir su ficha completa:
...
- **Actividad reciente**: los últimos 20 movimientos registrados — para saber de un vistazo si el cliente realmente está usando el sistema.
...
No. Solo puede registrar movimientos (entradas/salidas/muestras) y crear productos nuevos. Editar campos existentes de un producto es exclusivo de dueños/gerentes, desde el dashboard.

**¿Qué pasa si dos operarios trabajan en la misma bodega a la vez?**
No hay problema — cada uno ve el mismo stock en tiempo real (el dashboard se refresca solo cada 5 segundos) y el botón "deshacer" de cada uno solo afecta sus propios movimientos.
